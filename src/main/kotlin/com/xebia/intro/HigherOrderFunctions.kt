...
fun Song.belongsToGenreAndReleasedInYear(selectedGenre: Genre, year: Int): Boolean =
    this.genre == selectedGenre && this.releaseDate.year == year
...
fun PlayStats.calculateRoyalties(song: Song): Double {
    val playCount = stats[song] ?: return 0.0
 ...
 */
fun aggregateRoyaltiesByUsingHigherOrderFunctions(songsLibrary: List<Song>, playStats: PlayStats, genre: Genre, year: Int): Double =
    songsLibrary
 ...
 */
fun aggregateRoyaltiesInOneIteration(songsLibrary: List<Song>, playStats: PlayStats, genre: Genre, year: Int): Double =
    songsLibrary
        .sumOf {
            if (it.belongsToGenreAndReleasedInYear(genre, year)) playStats.calculateRoyalties(it)
            else 0.0
        }
